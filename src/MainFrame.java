...
public class MainFrame extends JFrame {
	...
	private JLabel stat_label = new JLabel("Your Statement");
	private String ID;
	...
	public void SetMainTable() throws SQLException {
		
		weather.setBounds(80, 60, weather.getWidth()+5, weather.getHeight()+5);
		...
		friend_label.setBounds(571, 280, 155, 20);
		...
		weather_label.setBounds(270, 280, 115, 20);
		...
		good_label.setBounds(510, 285, 150, 20);
		...
		history_label.setBounds(500, 385, 115, 20);
		...
		stat_label.setBounds(586, 155, 110, 20);
		...
		stat.setBounds(487, 87, img_stat.getWidth(), img_stat.getHeight());
		if(first.getFirst()==1) {
			friend.setActionCommand("First");
		}
		else {
			...
			public void actionPerformed(ActionEvent event) {
				...
				if (command.equals("Good")) {
					...
					try {
						new Friend_List(ID);
						mainframe.setVisible(false);
						JOptionPane.showMessageDialog(null,"ID可使用 '123'測試");
					} catch (SQLException e) {
						e.printStackTrace();
					}
		...
		weather.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				try {
					int op = JOptionPane.showConfirmDialog(null, "是否要前往中央氣象局?", "提示", JOptionPane.YES_NO_OPTION);;
					if (op == JOptionPane.YES_OPTION) {
	...
	public void LoadMainFile() {
		...
		java.net.URL imgURL;
		if (s.getBGPath().equals("/m.png")) {
			statPath = "sm.png";
		} else if (s.getBGPath().equals("/b1.png")) {
			statPath = "/sb1.png";
